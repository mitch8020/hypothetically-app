...
describe('runtime entrypoints', () => {
  ...
  it('covers the fallback branches of TypeScript decorator metadata helpers', () => {
    const reflect = Reflect as unknown as { metadata?: unknown };
    const previousMetadata = reflect.metadata;
    const runtime = globalThis as unknown as {
      Promise?: PromiseConstructor;
    };
    const previousPromise = runtime.Promise;
    ...
    try {
      ...
      jest.isolateModules(() => {
        ...
        jest.dontMock('./questions/question-generation.service');
      ...
      jest.isolateModules(() => {
        ...
        jest.doMock('./questions/dto/submit-answer.dto', () => ({
          SubmitAnswerDto: undefined,
        }));
        ...
        try {
          ...
          runtime.Promise = previousPromise;
        }
      });
      jest.isolateModules(() => {
        const configModule = jest.requireActual<typeof import('@nestjs/config')>(
        ...
        const mongoose = jest.requireActual<typeof import('mongoose')>(
          'mongoose',
        );
        jest.doMock('@nestjs/config', () => ({
          ...configModule,
        ...
        jest.doMock('./questions/question-generation.service', () => ({
          QUESTION_PROMPT_VERSION: 'coverage-test',
          QuestionGenerationService: undefined,
        }));
        jest.dontMock('./questions/questions.service');
        jest.dontMock('./questions/traffic.service');
        jest.dontMock('./users/users.service');
        require('./questions/questions.service');
